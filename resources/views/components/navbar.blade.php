        ...
        @else
            ...
            @if (Auth::user()->role === 'pasien')
                ...
                <li class="relative md:ml-4 group">
                    ...
                    <div class="hidden absolute left-0 md:right-0 md:left-auto top-full mt-0 w-48 bg-white rounded-lg shadow-lg py-2 z-50 group-hover:block">
            ...
            @elseif(Auth::user()->role === 'mitra' || Auth::user()->role === 'dokter')
                ...
                <li><a href="{{ route('mitra.welcome') }}" class="block px-6 py-3 md:p-0 hover:text-yellow-700">Beranda</a></li> {{-- Menggunakan route mitra.welcome dari main --}}
                <li><a href="{{ route('mitra.rujukan') }}" class="block px-6 py-3 md:p-0 hover:text-yellow-700">Rujukan</a></li> {{-- Menggunakan route mitra.rujukan dari main --}}
            ...
            @elseif(Auth::user()->role === 'admin')
                {{-- Tautan untuk Admin (Dari main, menggunakan route()) --}}
                <li><a href="{{ route('admin.dashboard') }}" class="block px-6 py-3 md:p-0 hover:text-yellow-700">Admin Panel</a></li>
                <li><a href="{{ route('admin.users') }}" class="block px-6 py-3 md:p-0 hover:text-yellow-700">Kelola Pengguna</a></li>

                 {{-- Bagian profil dropdown untuk Admin (Mirip Pasien, tapi mungkin hanya Logout) --}}
                 {{-- Asumsi admin juga butuh dropdown profil, disamakan dengan mitra/dokter --}}
                 <li class="relative md:ml-4 group">
                    <button type="button"
                        class="flex items-center gap-2 w-full px-6 py-3 md:p-0 hover:text-yellow-700 focus:outline-none">
                        <img src="https://ui-avatars.com/api/?name={{ urlencode(Auth::user()->name) }}&background=0D8ABC&color=fff"
    ...
    <div id="mobile-menu" class="hidden md:hidden absolute top-16 left-0 w-full bg-primary shadow-lg py-2 z-40">
         <ul class="flex flex-col font-medium">
            @guest
                ...
                <li><a href="{{ route('home') }}" class="block px-6 py-3 hover:bg-yellow-700 hover:text-white">Beranda</a></li>
                <li><a href="{{ route('tentang') }}" class="block px-6 py-3 hover:bg-yellow-700 hover:text-white">Tentang</a></li>
                <li><a href="{{ route('reservasi') }}" class="block px-6 py-3 hover:bg-yellow-700 hover:text-white">Reservasi</a></li>
                <li><a href="{{ route('login') }}" class="block px-6 py-3 hover:bg-yellow-700 hover:text-white">Akun</a></li>
            @else
                ...
                @if (Auth::user()->role === 'pasien')
                    ...
                    <li><a href="{{ route('reservasi') }}" class="block px-6 py-3 hover:bg-yellow-700 hover:text-white">Reservasi</a></li>
                    <li><a href="{{ route('riwayat') }}" class="block px-6 py-3 hover:bg-yellow-700 hover:text-white">Riwayat</a></li>
                    <li><a href="{{ route('profile.edit') }}" class="block px-6 py-3 hover:bg-yellow-700 hover:text-white">Edit Profil</a></li>
                 @elseif(Auth::user()->role === 'mitra' || Auth::user()->role === 'dokter')
                     {{-- Tautan Mobile untuk Mitra/Dokter --}}
                    <li><a href="{{ route('mitra.welcome') }}" class="block px-6 py-3 hover:bg-yellow-700 hover:text-white">Beranda Mitra</a></li>
                    <li><a href="{{ route('mitra.rujukan') }}" class="block px-6 py-3 hover:bg-yellow-700 hover:text-white">Rujukan</a></li>
                 @elseif(Auth::user()->role === 'admin')
                     {{-- Tautan Mobile untuk Admin --}}
                    <li><a href="{{ route('admin.dashboard') }}" class="block px-6 py-3 hover:bg-yellow-700 hover:text-white">Admin Panel</a></li>
                    <li><a href="{{ route('admin.users') }}" class="block px-6 py-3 hover:bg-yellow-700 hover:text-white">Kelola Pengguna</a></li>
                @endif
                <hr class="border-t border-gray-700 my-1"> {{-- Pembatas --}}
                 ...
                 <li>
                     <form method="POST" action="{{ route('logout') }}">
                         @csrf
                         <button type="submit" class="block w-full text-left px-6 py-3 text-gray-800 hover:bg-yellow-700 hover:text-white">
                             Logout
                         </button>
                     </form>
                 </li>
