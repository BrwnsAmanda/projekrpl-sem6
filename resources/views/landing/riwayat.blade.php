...
<body class="bg-cream font-sans font-poppins min-h-screen flex flex-col"> {{-- Tambah flex layout agar footer di bawah --}}
    ...
    <div class="max-w-5xl mx-auto py-6 px-2 sm:px-4 flex-1 w-full">
        <h2 class="text-xl sm:text-2xl font-bold mb-6">Riwayat Pemeriksaan</h2>
        ...
        <div class="grid gap-6">
            @forelse ($riwayats as $riwayat)
                ...
                <div class="bg-white border-2 border-primary rounded-xl shadow p-4 sm:p-6 flex flex-col gap-4">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            <div class="mb-2"><span class="font-semibold">Nama</span> :
                                {{ $riwayat->reservasi->nama ?? '-' }}</div>
                            <div class="mb-2"><span class="font-semibold">Tanggal Lahir</span> :
                                {{ $riwayat->reservasi->tanggal_lahir ?? '-' }}</div>
                            <div class="mb-2"><span class="font-semibold">Jenis Kelamin</span> :
                                {{ $riwayat->reservasi->jenis_kelamin ?? '-' }}</div>
                            <div class="mb-2"><span class="font-semibold">Alamat</span> :
                                {{ $riwayat->reservasi->alamat ?? '-' }}</div>
                            <div class="mb-2"><span class="font-semibold">No. NIK</span> :
                                {{ $riwayat->reservasi->nik ?? '-' }}</div>
                            <div class="mb-2"><span class="font-semibold">No. Telephone</span> :
                                {{ $riwayat->reservasi->telepon ?? '-' }}</div>
                            {{-- Tambah email jika ada di model Reservasi berelasi dengan User --}}
                            <div class="mb-2"><span class="font-semibold">Email</span> :
                                {{ $riwayat->reservasi->user->email ?? '-' }}</div>

                            {{-- Detail Pemeriksaan dan Tanggal (dari relasi reservasi) --}}
                            <div class="mt-4 font-semibold">Pemeriksaan:
                                {{ $riwayat->reservasi->jenis_pemeriksaan ?? '-' }}</div>
                            <div class="text-sm text-gray-600">Tanggal:
                                {{ $riwayat->reservasi->jadwal_pemeriksaan ?? '-' }}</div>
                        </div>
                        ...
                        <div class="flex flex-col items-end justify-start gap-2">
                            ...
                            <button onclick="previewInvoice({{ $riwayat->id }})"
                                class="bg-primary text-black px-4 py-2 rounded-lg font-semibold hover:bg-yellow-100 shadow w-full md:w-auto text-center text-sm sm:text-base">
                                Preview Invoice
                            </button>
                    ...
                    <div id="modal-invoice-{{ $riwayat->id }}"
                        ...
                        <div class="bg-white rounded-xl shadow-lg p-4 sm:p-6 relative max-w-lg w-full mx-2 sm:mx-4">
                            {{-- Atur lebar dan margin --}}
                            ...
                            <div class="bg-white rounded-xl shadow p-6 max-w-lg mx-auto border border-gray-300"
                                ...
                                <div class="flex justify-between items-center mb-4">
                                    <div>
                                        ...
                                        <div class="text-sm text-gray-500">No:
                                            {{ $riwayat->invoice_number ?? 'INV-' . str_pad($riwayat->id, 3, '0', STR_PAD_LEFT) }}
                                        </div>
                                    </div>
                                ...
                                <div class="mb-2">
                                    ...
                                    <div class="font-semibold">Tanggal: <span
                                            class="font-normal">{{ $riwayat->reservasi->jadwal_pemeriksaan ?? '-' }}</span>
                                    </div>
                                </div>
                                <hr class="my-2">
                                <div class="mb-2">
                            ...
                            <div class="flex flex-col sm:flex-row justify-end mt-4 gap-2">
                                <button onclick="downloadInvoiceAsPDF({{ $riwayat->id }})"
                                    class="bg-secondary text-white px-4 py-2 rounded text-sm sm:text-base hover:bg-secondary/80">Unduh
                                    PDF</button>
                                <button onclick="downloadInvoiceAsJPG({{ $riwayat->id }})"
                                    class="bg-primary text-black px-4 py-2 rounded text-sm sm:text-base hover:bg-yellow-100">Unduh
                                    JPG</button>
                            </div>
                        </div>
                    </div>
            ...
            @empty
                ...
                <div class="bg-white rounded-xl shadow-lg p-6 sm:p-8 text-center border-l-8 border-secondary">
                    ...
                    <p class="text-gray-600 mb-6">Anda belum memiliki riwayat pemeriksaan. Silakan lakukan reservasi
                        untuk
